...
public class BinarySearchTree<T extends Comparable<T>> {
	...
	private TreeNode add(T val, TreeNode tree) {
		if (tree == null) 
	...
	private T semiNullCheck(T a, T b) {
		if (a == null)
			return b;
	...
	public TreeNode searchForNode(T searchFor) {
		TreeNode result = search(root, searchFor);
		return result == null ? null : result.getValue().equals(searchFor) ? result : null;
	}

	@SuppressWarnings("unchecked")
	private TreeNode search(TreeNode node, final T searchFor) {
		if (node == null)
			return null;
	...
	@SuppressWarnings("unchecked")
	private TreeNode remove(TreeNode node, final T value) {
		if (node == null)
			return null;
		...
		if (compare < 0)
			...
			// System.out.println(node.getValue() + " -> " + next.getValue());
